...
bash tests/run_all.sh          # 48/48 controls — ~20 s (52 with RUN_SLOW=1)
...
```bash
bash tests/run_all.sh        # 48/48 controls — families below, ~20 s on a laptop (52 with RUN_SLOW=1)
```
...
| [RELEASE.md](RELEASE.md) | v0.2.6 wheel=HEAD same-day patch (explain crash-family) · v0.2.5 fresh-user audit release (epoch-verify in wheel) · v0.2.4 ingest-in-wheel + AT-026 packaging control · v0.2.3 project-head + PUGIO receiver · v0.2.2 wheel-side explain + keccak fix · v0.2.1 license fix · v0.2.0 FINAL const flip |
