...
negatif vektörlerle çalıştırır (`bash tests/run_all.sh` → 48/48):
...
**Tek-komut regresyon:** `bash tests/run_all.sh` — 48/48 kontrol (~20 sn; `RUN_SLOW=1` ile
...
| [RELEASE.md](RELEASE.md) · [CHANGELOG.md](CHANGELOG.md) | Sürüm-yüzeyleri ve değişim-günlüğü (0.2.7: tüm-CLI usage-guard + CR-v0.1 cross-proof; 0.2.6: aynı-gün yama — explain crash-family; 0.2.5: taze-kullanıcı denetimi — epoch-verify wheel'de; 0.2.4: ingest wheel'de + AT-026) |
...
bash tests/run_all.sh    # 48/48 kontrol — ~20 sn (RUN_SLOW=1 ile 52)
```
