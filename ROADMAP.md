...
- **İş:** test-önce dikey dilimler: (1) manifest+runner koşum **✔ kanıtlı** (kanit/FAZ1/2026-09-02/dilim-1.log), (2) bağlam grafiği+şifreli snapshot **✔ kanıtlı** (dilim-2.log), (3) taşıma **✔ kanıtlı** (dilim-2/3.log), (4) muhasebe+simüle ödeme **✔ omurga kanıtlı** (RFC-003 bekliyor), (5) AT-001 tamamı — **✅ PASS 2026-09-02 (a 6/6 + b/c/d/e; kanit/AT-001/2026-09-02/)**, (6) katılım yüzeyi: halka açık repo + build-in-public simnet logları. **Dilim-3 ✔:** gerçek WASI 0.3 koşumu — wasmtime v48.0.1 (digest-doğrulamalı), süreç-izole, D4 default-deny; motor: `tools/bin/wasmtime`; ajan: `tests/agent-src/`. Güvenlik denetimi: SECURITY-AUDIT.md (Audit-1/2/3/4) — her dilim sonunda denetleme kuralı işliyor. **Dilim-5 ✔:** ledger hash-zinciri + `grant`/`ledger-verify` (RFC-003 D4/D7 taslak gerçeklemesi) — kanit/FAZ1/2026-09-02/dilim-5.log. **Dilim-6/7/8 ✔ (2026-09-03):** F21 truncate-panzehiri (`ledger_tip` state'e yazılıyor, reason 14) · çapraz-import RED kanıtı · **F24 kapanışı: ledger snapshot'la seyahat eder** (gömülü `ledger_records` + tip bağlama) · merkle kurcalama RED (reason 17) · tek-komut regresyon takımı `tests/run_all.sh` (16 kontrol: AT-001f + AT-003 negatif vektör bölümleri dahil, POSIX çıkış-semantiği, PIPESTATUS ile boru-koruma) — kanit/REGRESYON/2026-09-03/. **Dilim-9 (2026-09-05):** AT-001f negatif vektör fabrikası — reason 7 (SAFE_SNAP_MAX aşımı) / 9 (header agent_id taklidi) / 8 (replay rollback) beklenmedik-KABUL kapanışı; kanit/AT-001/2026-09-05/AT-001f-vektorler.log. **Dilim-10 (2026-09-05):** node-cosign L1 pilot (F25 mekanizma kapanışı; AT-003 6/6 + Audit-8) — RFC-003 §8 D8 kurucu-onay bekliyor. RFC-002 E-9/E-10/E-11 errata'ları işlendi (2026-09-05). **RFC-003/004 TASLAK'ta — kurucu onayı bekliyor.**
...
- **Overhead taban çizgisi (2026-09-05, E-4; Audit-9 B2 düzeltmesiyle):** koşum-dışı op yükleri ölçüldü (kanit/BENCH/2026-09-05/runner-overhead.json — v2): medyan grant=217ms · ledger-verify(5-kayıt)=195ms · memory-search=200ms · export=356ms (scrypt baskın) · **import(derin-doğrulama)=421ms** (en ağır op: scrypt+AEAD+zincir-doğrulama+kuruluş). İlk ölçüm hatalıydı (import fixture'ı bench'ten SONRA kurulunca erken-RED yolu ölçülüyordu — 72ms yanıltıcı; hatalı sürüm git geçmişinde 89936d9'da arşivli). Yöntem notu: yüklü ortamda mutlak değer değil **op-arası oran** anlamlı; Faz 2'de sessiz host'ta tekrarlanıp "overhead < X% koşum" beyanı bu tabanla yapılır.
