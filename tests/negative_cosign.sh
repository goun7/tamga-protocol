...
#         L0 back-compat (a chain without node_sig).
...

{
  ...
  rm -rf "$SB"; mkdir -p "$SB/pkg" "$SB/pkg-clean" "$SB/freshL0" "$SB/freshL1" "$SB/freshL1b"
  for d in pkg pkg-clean freshL0 freshL1 freshL1b; do cp tests/vectors/tc-a1/tamga.json tests/vectors/tc-a1/agent.wasm "$SB/$d/"; done
...
PY
  ...
  python3 tamga_runner.py grant "$SB/pkg-clean" 0.01 "at003b" --node-key "$NK" > /dev/null
  python3 - "$SB/pkg-clean/ledger.jsonl" <<'PY'
import sys, json, pathlib
p = pathlib.Path(sys.argv[1])
...
PY
  python3 tamga_runner.py ledger-verify "$SB/pkg-clean" | grep -q '"ok": false'
  bekle_red $? "tc-n3: node_id swap → broken chain hash (RED)"

  # tc-n4: node_sig-less (clean legacy) chain + L1 → RED (node_sig_missing)
  ...
  python3 tamga_runner.py import "$SB/legacy.tsg" "$SB/freshL0" | grep -q '"ok": true'
  kontrol $? "tc-n6: L0 default legacy chain → ACCEPT (back-compat preserved)"

  rm -rf "$SB"
  echo ""
  echo "RESULT(at003): $PASS PASS, $FAIL FAIL"
  [ "$FAIL" = "0" ]
} 2>&1 | tee -a "$LOG"
exit ${PIPESTATUS[0]}
