...

{
  ...
  echo "--- AT-001f: import negatif vektörleri (reason 7/9/8) — ayrıntı: .evidence/AT-001/$(date +%F)/AT-001f-vektorler.log"
  bash tests/negative_snapshots.sh > /dev/null 2>&1; kontrol $? "tc-s7/s9/s8 negatif vektörleri (3 RED beklentisi)"

  echo "--- AT-003: node-cosign negatif vektörleri (F25 çözümü) — ayrıntı: .evidence/AT-003/$(date +%F)/AT-003-cosign.log"
